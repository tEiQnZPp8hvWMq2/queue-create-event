...
const packageObjectAdmin = grpcLibrary.loadPackageDefinition(packageDefinition).aff_transaction_sum;
...
const ServiceClient = new packageObjectAdmin.AffTransactionSumService(process.env.GRPC_AFF_PLAYER_URL,credentials,{
...
module.exports  = {
    ...
    Query: async function(messages) {
        return new Promise((resolve, reject) => {
            client.Query({ request : messages }, (err, response) => {
                if (err) return reject(err);
